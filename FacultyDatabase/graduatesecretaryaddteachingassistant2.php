...
<?php
   ...
   $userID=$_POST["userid"];
   $degree=$_POST["degree"];
   $headSuperVisor=$_POST["headsupervisor"];   

   $query = 'insert into teachingassistant values("' . $firstName . '","' . $lastName . '","' . $studentNumber . '","' . $userID . '","' . $degree . '","' . $tapicture . '","' . $headSuperVisor . '")';
   $result=mysqli_query($connection, $query);
   if (!$result) {
        die("Error: insert failed");
    }
   
   echo $firstName . " " . $lastName . " was added as a Teaching Assistant." . "<br><br>";
